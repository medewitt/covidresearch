plotFig <- function(c1, c2, week, sameScale = TRUE, byRec =100000, thedate = Sys.Date(), newdat){
...
raw_data <- data.table::fread("https://raw.githubusercontent.com/flodebarre/covid_vaccination/25ac4e084abf24f97a56cd7889b05ebd9888e0e2/ecdc-vaccination/data/ecdc-vaccination_cumulated_2021-09-08.csv")

plotFig("ES", "FR", week = 32, sameScale = TRUE,newdat = raw_data)

names(raw_data)

nc_dat <- nccovid::get_vaccinations_demo(demographic = "age")
nc_dat[,minAge:=as.numeric(stringr::str_extract(demographic_identity, "(\\d+)[^-]"))]
nc_dat[,maxAge:=as.numeric(stringr::str_extract(demographic_identity, "[^-](\\d+)"))]
nc_dat[,Population := sum(county_demo_pop, na.rm = TRUE), by = c("county", "week_of")]
nc_dat[,ageWidth:=maxAge-minAge+1]
nc_dat[,Denominator:=county_demo_pop]
nc_dat[,FirstDose:=]